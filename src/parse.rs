...
mod tests {
    ...
    fn file_path_regex_rejects_version_dot_fragments() {
        ...
        assert!(
            result
                .file_refs
                .contains(&"formal-model/large-corpus-algebra-v1.2.md".to_string())
                || result.file_refs.is_empty(),
    ...
    fn parallel_run_large-corpus() {
        let home = std::env::var("HOME").expect("HOME must be set");
        let corpus_path = format!("{home}/code/large-corpus/.design/");
        let (old_total, _new_total, _fewer, _more, total_body, body_with_span) =
            parallel_run_corpus(&corpus_path, "Large Corpus (.design/)");
    ...
    fn parallel_run_host-corpus() {
        let home = std::env::var("HOME").expect("HOME must be set");
        let corpus_path = format!("{home}/code/host-corpus/.design/");
        let (old_total, _new_total, _fewer, _more, total_body, body_with_span) =
            parallel_run_corpus(&corpus_path, "Host Corpus (.design/)");
