...
Implement the four deterministic transforms first. They cover the actual false positives observed in Host Corpus (50 errors) and Large Corpus (186 errors). Add strsim only if deterministic candidates miss common cases.
...
- Host Corpus: frontmatter URLs and prose false positives (ignore_refs covers URL-like patterns)
- Large Corpus: terminal-file noise (already handled by `--active-only`; `ignore_files` extends this)
