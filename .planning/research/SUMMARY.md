...
5. **Active-only default change (P10)** — Do NOT change the `--active-only` default. Breaks CI scripts (186 errors -> 9 on Large Corpus), poisons `history.jsonl` snapshots with a discontinuity, makes `anneal diff` report false improvement. Add config opt-in `[check] default_filter = "active-only"` instead.
...
**Needs phase research:** YES. Needs regression testing against Large Corpus (262 files) and Host Corpus corpora. Diagnostic count changes expected but must be characterized. Parallel-run old vs new scanner during development.
...
- **Regression characterization:** Before Phase 3, need baseline diagnostic counts from Large Corpus and Host Corpus with the old scanner. Compare after migration. This is implementation work, not research.
...
- Live `anneal check --json` output from Large Corpus corpus -- confirmed exact JSON field names and types
