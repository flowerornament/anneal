...
**Detection:** Search test corpora for HTML constructs. If any exist, this pitfall is active. The Large Corpus corpus likely has HTML comments; the Host Corpus corpus may have `<details>` blocks.
...
**Prevention:**
- Run both scanners in parallel during development and diff their outputs on the Large Corpus and Host Corpus corpora.
...
**Detection:** Run on Host Corpus corpus (89 errors -> 50 with --active-only). Check that suggestions are actionable, not noise.
...
**What goes wrong:** v1.1 wants `--active-only` to become the default behavior for `anneal check`. Currently, the default shows ALL diagnostics (confirmed live: 186 errors on Large Corpus with 177 from terminal files). Changing the default means scripts that parse error counts see a dramatic drop (186 -> 9 on Large Corpus). A CI pipeline asserting `errors == 0` would suddenly pass (false green). More insidiously, convergence tracking via `history.jsonl` snapshots sees a discontinuity -- the snapshot error count drops, making `anneal status` report the corpus is "advancing" massively when it's just a filter change.
...
**What goes wrong:** Adding source locations, candidates, and snippets to human-readable output makes each diagnostic 4-6 lines instead of 2. On Large Corpus (186 errors), that's 750+ lines of output. Terminal scrollback loses the summary line.
...
- Live `anneal check --json` output from Large Corpus corpus: confirmed exact field names and types in JSON contract (HIGH confidence)
