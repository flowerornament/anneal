...
### Claude's Discretion
- Keyword proximity rule for body text inference (same-line vs same-paragraph) -- Claude should choose based on what works best during implementation and testing against the Large Corpus corpus.
...
- **Test corpus:** `/path/to/large-corpus/.design/` (283 files, 15+ namespaces)
...
**What goes wrong:** File path references like `formal-model/sample-formal-model-v11.md` are relative to the referring file's directory, not the root. Getting this wrong breaks File handle resolution.
...
**How to avoid:** Per CONTEXT.md (Claude's discretion), start with same-line proximity and test against Large Corpus corpus. If too restrictive, expand to same-paragraph. The keyword must be on the same line as the reference for the edge kind to override the default Cites.
...
**What goes wrong:** The scanner reads every line of every file. Some files in Large Corpus are very large (formal model v17 is thousands of lines).
...
1. **Keyword proximity for body-text edge inference**
   ...
   - Recommendation: Start with same-line (simplest, most precise). Test against Large Corpus corpus. Expand to paragraph if too restrictive. This is explicitly Claude's discretion per CONTEXT.md.
...
2. **Version handle discovery scope**
   ...
   - Recommendation: Match `v\d+` only in file names and in frontmatter `version:` fields. Body text `v17` is too noisy (Large Corpus has 861 matches for `v1` alone). The primary signal is the filename pattern `*-vN.md` and the frontmatter `version:` field.
...
| Large Corpus corpus | Integration testing | Yes | 283 .md files at /path/to/large-corpus/.design/ | -- |

**Missing dependencies with no fallback:** None.

**Missing dependencies with fallback:** None.

## Corpus Analysis (Large Corpus .design/)
...
- `/path/to/large-corpus/.design/` -- live corpus probed for actual label counts, status values, directory structure
