...
provides:
  ...
  - "Integration test validating Large Corpus corpus (259 files, 9788 handles, 22 namespaces)"
...
**Namespace inference with false positive rejection, version handle supersession chains, and end-to-end CLI pipeline producing 9788 handles and 6408 edges from 259 Large Corpus files in <200ms**
...
- Integration test validates Large Corpus corpus: 259 files, 9788 handles, 6408 edges, 22 namespaces (OQ, FM, A, SR, DG, etc.), zero false positives (SHA, AVX, GPT correctly rejected)
...
2. **Task 2: Wire main.rs pipeline and validate against Large Corpus corpus** - `60dda48` (feat)
...
- `src/main.rs` - CLI with clap derive (--root, --json), full pipeline wiring, GraphSummary JSON output, human-readable print, Large Corpus integration test
...
- Version handles are created from filename patterns only (`*-vN.md`), not from body text `v17` references (too noisy -- Large Corpus has 861 matches for `v1` alone)
...
- Large Corpus corpus validated: 259 files, 9788 handles, 6408 edges, 22 namespaces
