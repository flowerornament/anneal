...
**Date:** 2026-03-29
**Corpus:** Large Corpus `.design/` (260 files, 120 with frontmatter)
**Output:** 9788 handles, 6408 edges, 22 namespaces, 39 version handles, 25 statuses

These observations come from manual testing against the Large Corpus corpus after Phase 1 execution. They document what Phase 2 planning needs to account for — none are Phase 1 bugs.
...
| Frontmatter superseded-by | `sample-formal-model-v15.md` | `formal-model/history/sample-formal-model-v15.md` | Never (bare name) |
| Frontmatter depends-on | (unused in Large Corpus) | — | Untested |
...
**Large Corpus reality:** At least "superseded" (5 files), "historical" (10), "archived" (1), "retired" (1), "incorporated" (11), "complete" (6) are terminal or near-terminal. The `archive/`, `history/`, and `prior/` directories provide strong signal.
...
## 3. Large Corpus Uses Rich Frontmatter That anneal Ignores

**Parsed by anneal (6 fields):** status, updated, superseded-by, depends-on, discharges, verifies

**Used in Large Corpus but not parsed (15+ fields with reference data):**
...
**Impact on Phase 2:** These fields are project-specific (Large Corpus conventions), not part of anneal's core model. CONFIG-03 could add extensible frontmatter field mapping. The `affects:` field is particularly valuable — it's the inverse of "depends-on" and appears in 32 files.
...
Version handles are created with `status: None` even when the underlying file has a status. For example, `sample-formal-model-v14.md` has `status: superseded` but its Version handle has no status. This means convergence tracking can't distinguish active from superseded versions through the Version handle — you'd need to traverse to the file handle.

## 7. File Path Regex Catches URLs

The pattern `[a-z0-9_/-]+\.md` matches URL fragments like `com/rust-lang/portable-simd/blob/master/beginners-guide.md`. Only 6 occurrences in Large Corpus — negligible. Could be fixed by requiring the match NOT be preceded by `://` or `http`.
