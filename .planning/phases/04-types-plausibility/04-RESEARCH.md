...
**Why it happens:** Overfitting to Host Corpus/Large Corpus false positives.
...
**Warning signs:** `anneal check` on Large Corpus shows FEWER total diagnostics than before (should show same number of real errors, just different classification for false positives).
...
### Example 1: Real False Positives from Host Corpus Corpus
...
E001: broken reference: specs/2026-03-21-host-corpus-next-era.md, specs/2026-03-21-architecture-theory.md not found

# Non-markdown paths -- informational, not broken references
E001: broken reference: lib/host-corpus/agent/ not found
E001: broken reference: lib/host-corpus/agent/session.ex not found
```

### Example 2: Real False Positives from Large Corpus Corpus

```
# Absolute root-prefixed paths -- should be RefHint::Implausible (or Phase 6 resolution)
E001: broken reference: .design/formal-model/sample-formal-model-compact-v8.md not found
...
1. **Non-markdown file paths in frontmatter (e.g., `lib/host-corpus/agent/session.ex`)**
...
- Pitfalls: HIGH - real false positives examined from Host Corpus and Large Corpus corpora (186 Large Corpus E001s, 50+ Host Corpus E001s analyzed)
