...
human_verification:
  - test: "Run anneal check on a corpus with URLs in frontmatter and verify no E001 for URL targets"
    ...
    why_human: "Needs a corpus with configured frontmatter fields pointing to URLs (large-corpus has none)"
---
...
| --- | ------------------------------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------- |
| 1   | `anneal check --json` output includes `DiscoveredRef` with `RefHint` classification              | ✓ VERIFIED  | `extractions` array present in JSON; 262 entries on large-corpus corpus; 62 with refs; Label/FilePath/Implausible hints confirmed |
| 2   | URLs in frontmatter edges appear as `RefHint::External` and do NOT produce E001                  | ✓ VERIFIED  | `classify_frontmatter_value` returns `External` for `https://`; 0 E001 for URL targets on large-corpus corpus |
| 3   | Absolute paths, freeform prose, wildcards produce W004 instead of false positive E001            | ✓ VERIFIED  | 18 W004 on large-corpus corpus (e.g. "spec/SPEC.md v0.8" → freeform prose); 5 integration tests pass       |
...
| ------------------- | ------------------ | ------------------------------------------------ | -------------------------- | --------------- |
| `src/parse.rs`      | `DiscoveredRef`    | `classify_frontmatter_value` per fe.targets loop | Yes — 253 refs on large-corpus   | ✓ FLOWING       |
| `src/parse.rs`      | `FileExtraction`   | Per-file in build_graph, one per markdown file   | Yes — 262 entries on large-corpus | ✓ FLOWING      |
| `src/cli.rs`        | `extractions`      | `result.extractions.clone()` from BuildResult    | Yes — in JSON output       | ✓ FLOWING       |
| `src/parse.rs`      | `implausible_refs` | `classify_frontmatter_value` returns Implausible | Yes — 18 W004 on large-corpus    | ✓ FLOWING       |
...
| ----------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------ | ------- |
| `extractions` array present in JSON output            | `anneal --root large-corpus/.design/ check --json` — check for `extractions` key          | Present; 262 entries                                   | ✓ PASS  |
...
| W004 still fires for implausible refs                 | Check W004 count in diagnostics                                                      | 18 W004 on large-corpus corpus                               | ✓ PASS  |
...
| ------------ | ----------- | ------------------------------------------------------------------------------------------------ | ------------- | ----------------------------------------------------------------------------------------------------------------- |
| EXTRACT-01   | 04-01/03    | Introduce `FileExtraction` as uniform extraction output from frontmatter and body scanning        | ✓ SATISFIED   | FileExtraction constructed per-file in build_graph; 262 entries on large-corpus; present in CheckOutput JSON           |
| EXTRACT-02   | 04-01/03    | Introduce `DiscoveredRef` with `RefHint` replacing `PendingEdge`/`LabelCandidate`/etc.           | ✓ SATISFIED   | DiscoveredRef populated for every frontmatter target in production; 253 refs on large-corpus; Label/FilePath/Implausible hints in JSON output. Note: runs alongside existing types (not replacing — replacement deferred to Phase 5 body scanner migration) |
| EXTRACT-05   | 04-02       | Plausibility filter rejects absolute paths, freeform prose, wildcards from frontmatter            | ✓ SATISFIED   | classify_frontmatter_value + build_graph loop + W004 + 5 integration tests + 18 W004 on large-corpus corpus             |
...
**Why human:** The large-corpus corpus has no frontmatter fields with URL values configured. The spot-check shows 0 External hints on large-corpus, so the External code path has not been exercised on a real corpus (only in unit tests).
...
**Verification on large-corpus corpus confirms:**
