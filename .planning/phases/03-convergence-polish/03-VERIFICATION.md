...
| 7  | anneal check --suggest/--stale/--obligations filter flags work correctly                      | VERIFIED   | `CheckFilters` struct in cli.rs; filters confirmed working against Large Corpus corpus     |
...
|----------------------------------------------------|---------------------------------------------------------------------|----------------------------------------------------------------------|--------|
| `anneal status` produces 7-line dashboard          | `anneal --root /path/to/large-corpus/.design/ status`                      | "Scanned: 261 files, 9836 handles, 6925 edges" (7 lines)           | PASS   |
...
| `anneal diff` shows changes since last snapshot    | `anneal --root /path/to/large-corpus/.design/ diff`                        | "Since last snapshot: Handles: +0..."                               | PASS   |
| `anneal map` renders text with Files/Labels groups | `anneal --root /path/to/large-corpus/.design/ map \| head -20`             | "Files (261):" with file list                                        | PASS   |
| `anneal map --format=dot` produces graphviz output | `anneal --root /path/to/large-corpus/.design/ map --format=dot \| head -3` | "digraph anneal { rankdir=LR; ..."                                  | PASS   |
| `anneal check --suggest` shows S001 diagnostics    | `anneal --root /path/to/large-corpus/.design/ check --suggest \| head -3`  | "suggestion[S001]: orphaned handle: ..."                            | PASS   |
| `anneal check --stale` shows W001 diagnostics only | `anneal --root /path/to/large-corpus/.design/ check --stale \| head -3`    | "warn[W001]: stale reference: ..."                                  | PASS   |
| `anneal status --json` produces valid JSON         | `anneal --root /path/to/large-corpus/.design/ status --json \| python3 -m json.tool` | Valid JSON with all expected fields                       | PASS   |
...
- Visual output quality confirmed via spot-checks against Large Corpus corpus
