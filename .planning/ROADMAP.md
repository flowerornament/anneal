...
**Success Criteria:**
1. Running anneal on Large Corpus's .design/ directory (265 files) produces a graph with ~500 handles and ~2000 edges in <100ms
...
**Success Criteria:**
1. `anneal check` on Large Corpus's .design/ reports real broken references and stale references with compiler-style diagnostics
...
4. `anneal init` generates an anneal.toml from inferred structure that matches Large Corpus's conventions
...
- **Phase 1 verification:** Build graph from Large Corpus's .design/, assert handle/edge counts, benchmark <100ms
