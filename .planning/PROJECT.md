...
anneal was designed through a deep design conversation exploring the formal analogy between graded type systems (TensorQTT from Large Corpus's formal model) and knowledge management. The core insight: a document's convergence state has the same algebraic structure as a program's resource grade — both are values in bounded lattices that compose through meet/join operations.

The design follows Host Corpus's coloring book principle (C-10): the kernel defines an abstract space (handles, graph, convergence lattice, local checks, linearity); each project is a coloring (which handles exist, which states are valid, which namespaces are linear).

The first corpus to test against is Large Corpus's own .design/ directory: 265 markdown files, 15 label namespaces, ~25 status values, machine-checked proofs in Agda/Lean.

Detailed specification: `.design/anneal-spec.md` (933 lines, 66 labels, 8 namespaces).

The primary test corpus is Large Corpus's `.design/` directory at `/path/to/large-corpus/.design/` (265 markdown files, 15 label namespaces). Integration tests point there via path, not structural coupling.
...
Phase 1 complete — graph foundation is working end-to-end. Running `cargo run -- --root /path/to/large-corpus/.design/` produces 9788 handles, 6408 edges, 22 namespaces from 259 files. The pipeline: parse files → infer namespaces → resolve labels/versions/edges → infer lattice → print stats. Quality gate (`just check`) passes.
