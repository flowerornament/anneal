...
#[cfg(test)]
use anneal_core::runtime::{Row, Statement, Value, analyze, parse_program};
#[cfg(test)]
use anneal_core::{CorpusId, FactStore, Generation};
...
#[cfg(test)]
use command::{OutputMode, OutputPreference, RootSelection};
use help::HelpTopic;
#[cfg(test)]
use help::{SKILL_MARKDOWN, skill_briefing_body, skill_section};
#[cfg(test)]
use navigation::handle_lineage_rows;
#[cfg(test)]
use navigation::{RESOLVED_FILE_META_KEY, SUPERSEDES_EDGE_KIND};
#[cfg(test)]
use output::{
    CommandOutput, EMPTY_ROWS_DIAGNOSTIC, RankedAnchorEnrichment, RowView, StatusOutput,
    eval_zero_result_hint, render_dynamic_verb_help, required_number, required_string,
    search_zero_result_hint, write_handle_text,
};
...
#[cfg(test)]
use session::{
    CHECK_DIAGNOSTIC_QUERY, DEFAULT_CORPUS, drift_refresh_progress_for,
    edge_assertion_refresh_progress_for, handle_query,
};
...
mod tests {
    use super::*;
    use anneal_core::runtime::Query;
    use anneal_core::runtime::eval::ExplainRowLimit;
    use anneal_core::runtime::prelude::standard_prelude_program;
    use anneal_core::{
        EdgeFact, FactBatch, FactBatchMode, FactIdentity, HandleFact, InferredCorpusRoot, MetaFact,
        NativeId, OriginUri, Revision, SourceName,
    };
    ...
    use crate::{ContextOutput, DEFAULT_READ_BUDGET, DEFAULT_SEARCH_LIMIT};
    use camino::Utf8Path;
