...
mod tests {
    use super::*;
    use crate::ast::{Expr, Literal, Term};
    ...
    fn load_include_and_import_resolves_predicates() {
        ...
        let names = program
            .rules()
            .map(|rule| rule.head.predicate.display_name())
            .collect::<Vec<_>>();
    ...
    fn import_preserves_references_to_global_predicates() {
        ...
        let imported = program
            .rules()
            .find(|rule| rule.head.predicate.display_name() == "strict.blocker")
            .expect("imported rule");
        let Atom::Derived(derived) = &imported.body.atoms[0] else {
            panic!("expected derived body atom");
        };
        assert_eq!(derived.predicate.display_name(), "active");
    }
    ...
    fn loaded_rules_preserve_child_file_source_locations() {
        ...
        let program = load_program(&root, "anneal.dl").expect("program loads");
        let rule = program.rules().next().expect("loaded child rule");
        let Atom::Derived(derived) = &rule.body.atoms[0] else {
            panic!("expected derived body atom");
        };
        assert_eq!(
            derived.location,
    ...
    fn prelude_loader_allows_reserved_diagnostic_ids() {
        ...
        let rule = program.rules().next().expect("diagnostic rule");
    ...
    fn test_root(_name: &str) -> TestRoot {
        TestRoot {
            dir: tempfile::tempdir().expect("create test root"),
        }
    }
