...
const AXIS_TOPIC_CARDS: &[AxisTopicCard] = &[
    ...
    AxisTopicCard {
        ...
        oracle: "source status values interpreted through sealed effective lifecycle classification; project settled entries do not imply terminality.",
        disposition: "REPORT / PRE-FLIGHT: declaring a status settled grants current-head anchor-boost eligibility and standing to report currency_suspect, but never asserts Supersedes.",
        ...
        common_joins: &[
            ...
            "`operative(h), *handle{id: h, file: file}` for settled handles eligible for current-head ranking boosts and currency_suspect standing",
