...
Cross-corpus testing on `/path/to/large-corpus/.design` and
`/path/to/host-corpus/.design` exposed a structural mismatch between that
formula and what orientation actually needs.

- **Large Corpus (12.6k handles):** the top-7 surfaced `DESIGN-GOALS.md`,
...
- **Host Corpus (smaller, denser):** the tool surfaced `system-theory.md`
  (March 24 axioms) and `host-corpus-architecture.md` (April 8 canonical
  ...
  10-host-corpus-architecture-synthesis.md` (frontmatter says "Entry point.
...
2. `status: living` (large-corpus's convention for "authoritative,
...
- The active-status set and terminal-status set are hardcoded from the
  ...
  not needed for the release — both large-corpus and host-corpus use standard
...
### S1. Fresh agent onboarding to large-corpus (no flags, budget=50k)

```
anneal --root /path/to/large-corpus/.design orient
```
...
### S2. Area-scoped work on large-corpus compiler

```
anneal --root /path/to/large-corpus/.design --area=compiler orient --budget=30k
```
...
### S3. Agent resuming on host-corpus

```
anneal --root /path/to/host-corpus/.design orient
```
...
`purpose: entry point`), `architecture/2026-04-10-host-corpus-architecture-
...
```
anneal --root /path/to/anneal/.design orient --budget=10k
```
...
- `orient_large-corpus_corpus_hits_readme_and_changelog`: confirm `README.md`
  and `CHANGELOG.md` appear in Foundation and that `2026-04-20`
  implementation files lead Frontier.
- `orient_host-corpus_corpus_surfaces_corpus_map`: confirm `synthesis/2026-
  04-15-corpus-map.md` and `architecture/2026-04-10-host-corpus-
  architecture-synthesis.md` appear.
- `orient_large-corpus_no_alias_stubs`: confirm none of
  `spec/SPEC.md`, `compiler/prior/bytecode-format-v1.md`,
  `spec/decisions-spec-v08.md`, `references/transcripts/2026-03-29-
  MANIFEST.md` appear.

Host Corpus and large-corpus tests use `#[ignore]` (skipped if corpus unavailable)
like the existing `test_large-corpus_corpus` integration.
...
- large-corpus/.design: `README.md` + `CHANGELOG.md` + `DESIGN-GOALS.md`
  ...
  1k READMEs.
- host-corpus/.design: `synthesis/2026-04-15-corpus-map.md` +
  `architecture/2026-04-10-host-corpus-architecture-synthesis.md` lead
...
- Research agent reports (Host Corpus and Large Corpus), 2026-04-21.
