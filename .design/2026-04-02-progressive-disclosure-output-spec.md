...
The audit in [2026-04-02-cli-output-audit.md](/path/to/anneal/.design/2026-04-02-cli-output-audit.md) shows that:

- `check --json` on the Large Corpus corpus emitted about `3.07 MB`
- `find "" --json` on the Large Corpus corpus emitted about `2.08 MB`
- `map --json` on the Large Corpus corpus emitted about `760 KB`
- `get LABELS.md --json` on the Large Corpus corpus emitted about `59 KB`
...
- [`print_json()`](/path/to/anneal/src/cli.rs#L94) uses `serde_json::to_string_pretty`
...
`emit_output()` in [main.rs](/path/to/anneal/src/main.rs#L481) must accept the JSON style and pass it through.
...
Extend the `Check` subcommand in [main.rs](/path/to/anneal/src/main.rs#L172) with:
...
In [main.rs](/path/to/anneal/src/main.rs#L655):
...
Extend `Get` in [main.rs](/path/to/anneal/src/main.rs#L217) with:
...
Extend `Find` in [main.rs](/path/to/anneal/src/main.rs#L239) with:
...
Extend `Map` in [main.rs](/path/to/anneal/src/main.rs#L329) with:
...
Update [main.rs](/path/to/anneal/src/main.rs) help text:
...
Update [anneal-spec.md](/path/to/anneal/.design/anneal-spec.md) after implementation is complete.
...
1. `check --json` on Large Corpus no longer emits MB-scale payloads by default.
