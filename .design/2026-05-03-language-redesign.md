...
An agent lands in `/path/to/anneal/.design/`. They've been told:
...
`large-corpus/.design/` + `host-corpus/.design/`. `--root=PATH` handles one corpus
at a time. Cross-corpus queries (*"large-corpus's OQ-12 cites anneal's
