...
Current (large-corpus, truncated):
...
Run on **two corpora** (`/path/to/anneal/.design` + `/path/to/large-corpus/.design`)
...
Populated by Phase 1 against two corpora (/path/to/anneal/.design,
/path/to/large-corpus/.design). Severity: `critical` (breaks parseability),
...
| F08 | check                | default on large-corpus            | 16 suggestions back-to-back, no visual grouping between severity classes                         | nit      | —    | Insert a blank line between severity groups (errors → info → suggestions)                   | closed |
...
| F16 | find                 | kw (large-corpus)                  | File column repeats same base path 18× when all hits are sections of one file                    | design   | —    | Out of scope (needs grouping feature); defer to Round 3                                      | defer  |
...
- Design source material: `/path/to/cli-design-notes/.agents/{cli-design-principles,
  ux-design-system}.md`
