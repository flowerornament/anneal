...
The primary test corpus is Large Corpus's `.design/` directory at `/path/to/large-corpus/.design/`:
- 265 markdown files
- 15 label namespaces (OQ, FM, A, SR, DG, ...)
- ~25 status values
- Machine-checked proofs in Agda/Lean

Integration tests point there via path: `anneal --root /path/to/large-corpus/.design/ check`
